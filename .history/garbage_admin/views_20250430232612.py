...
from .forms import SignUpForm, ProfileForm, TokenTransactionForm
    ...
    if request.user.is_authenticated:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                if token_transactions.transaction_type == 'credit':
...
def buy_tokens(request):
    if request.user.is_authenticated:
        ...
        if request.method == 'GET':
            ...
            if form.is_valid():
                token_transaction.amount = form.cleaned_data['amount']
                token_transaction.transaction_type = 'credit'
                token_transaction.user = current_user
                token_transaction.save()
                type(form.cleaned_data['amount'])
                current_profile.tokens += form.cleaned_data['amount']
                current_profile.save()
                messages.success(request, f'You Have Successfully Bought Tokens')
                return redirect('garbage_admin_profile')
            else:
                return render(request, 'buy_token.html', {'form':form})
    else:
        return render(request, 'login.html', {})
...
def token_transactions(request):
    if request.user.is_authenticated:
        current_user = User.objects.get(username=request.user)
        current_profile = Profile.objects.get(user=current_user)
        token_transactions = TokenTransaction.objects.filter(user=current_user)
        return render(request, 'token_transactions.html', {'current_user':current_user, 'current_profile':current_profile, 'token_transactions':token_transactions})
    else:
        return render(request, 'login.html', {})