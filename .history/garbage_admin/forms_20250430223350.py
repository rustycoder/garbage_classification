...
from garbage_admin.models import Profile
...
class BuyTokenForm(ModelForm):

	amount = forms.CharField(label="", min_length=10, max_length=10, widget=forms.TextInput(attrs={'class':'form-control', 'placeholder':'Amount'}), help_text='<span class="form-text text-muted"><small>Phone number should be 10 digit.</small></span>')
	type = forms.CharField(label="", max_length=100, widget=forms.TextInput(attrs={'class':'form-control', 'placeholder':'Type'}))
    ...
    def __init__(self, *args, **kwargs):
        self.user = kwargs.pop('user')  # Expect user to be passed in the view
    ...
    def save(self, commit=True):
        ...
        if commit:
            ...
            # Optionally update user profile credits
            profile = self.user.profile
            profile.credits += transaction.amount
            profile.save()
